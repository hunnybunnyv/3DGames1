...
function createGround(scene) {
    ...
    function onGroundCreated() {
        ...
        groundMaterial.diffuseTexture = new BABYLON.Texture("images/ice.jpg");
...
function createFollowCamera(scene, target) {
    ...
    camera.radius = 90; // how far from the object to follow
...
function createTank(scene) {
    ...
    // let tankPhys = new BABYLON.PhysicsImpostor(tank, BABYLON.PhysicsImpostor.BoxImpostor, {mass: 1, restitution: 0.9}, scene);
    ...
    tank.fireCannonBalls = function() {
        ...
        {
            // Modèles "Dame".
            scene.dames.forEach( actor =>
            {
                cannonball.actionManager.registerAction( new BABYLON.ExecuteCodeAction(
                    {
                        trigger: BABYLON.ActionManager.OnIntersectionEnterTrigger,
                        parameter: actor.Dame.bounder
                    },
                    () =>
                    {
                        actor.Dame.bounder.dispose();
                        actor.dispose();
                    }
                ) );
            } );
...
function createDame(scene) {
    // load the Dude 3D animated model
     // name, folder, skeleton name 
     BABYLON.SceneLoader.ImportMesh("dame", "models/Dame/", "dame.babylon", scene,  (newMeshes, particleSystems, skeletons) => {
         let dame = newMeshes[0];
         dame.position = new BABYLON.Vector3(0, 0, 5);  // The original dame
         // make it smaller 
         //heroDude.speed = 0.1;
         ...
         let hero2 = new Dame(dame, -1, 0.1, 0.2, scene);
 
         // make clones
         scene.dames = [];
         for(let i = 0; i < 10; i++) {
             scene.dames[i] = doClone(dame, skeletons, i);
             scene.beginAnimation(scene.dames[i].skeleton, 0, 120, true, 1);
 
             // Create instance with move method etc.
             // params = speed, scaling, scene
             new Dame(scene.dames[i], i, 0.3, 0.2, scene);
         ...
         scene.dames.push(dame);
 
     },
     (evt) => {
        // error occurred
        console.log("Error occurred while loading dame.babylon");
        console.log(evt);
    });
 }
...
function createHeroDude(scene) {
   // load the Dude 3D animated model
    ...
    BABYLON.SceneLoader.ImportMesh("him", "models/Dude/", "Dude.babylon", scene,  (newMeshes, particleSystems, skeletons) => {
        ...
        heroDude.position = new BABYLON.Vector3(10, 0, 40);  // The original dude
        // make it smaller 
        //heroDude.speed = 0.1;
...
function moveOtherDudes() {
    ...
    if(scene.dames) {
        for(var i = 0 ; i < scene.dames.length ; i++) {
            scene.dames[i].Dame.move(scene);
        }
    }   
}
